...
if ($busqueda !== "") {
    ...
    $where[] = "or=(nombre.ilike.$texto,correo.ilike.$texto)";
}

// Construir query
$query = "usuarios?select=*&order=id_usuario.asc";
...
// =====================================
list($code, $usuarios, $total) = supabase_get($query);
        ...
        <div class="overflow-x-auto">
            ...
            <div class="px-6 py-4 flex justify-between items-center text-sm">
                ...
                <span class="text-gray-600">
                    Mostrando <?= count($usuariosPagina) ?> de <?= $totalRegistros ?> usuarios
                </span>

                <div class="flex gap-2">
                    <!-- Anterior -->
                    <?php if ($paginaActual > 1): ?>
                        <a href="?pagina=<?= $paginaActual - 1 ?>&filtro=<?= $filtro ?>&buscar=<?= urlencode($busqueda) ?>"
                    ...
                    <?php for ($i = 1; $i <= $totalPaginas; $i++): ?>
                        <a href="?pagina=<?= $i ?>&filtro=<?= $filtro ?>&buscar=<?= urlencode($busqueda) ?>"
